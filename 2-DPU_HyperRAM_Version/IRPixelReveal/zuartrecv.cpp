...
ZUARTRecv::ZUARTRecv(QObject *parent) : QObject(parent)
{
  ...
  this->m_uart->setBaudRate(1000000); //1Mbps.
  ...
  if(!bRet)
    {
      ...
      emit this->ZSignalLog(this->m_uart->portName()+",1Mbps/8N1");
...
void ZUARTRecv::ZSlotDataReady()
{
  ...
  if(iSpaceRemain>=iRdMax)
    {
      ...
      emit this->ZSignalLog("Buffer overflow, reset.");
      this->m_ImgLen=0;
      emit this->ZSignalRxBytes(this->m_ImgLen);
      return;
    }
  ...
  if(!iSyncHeadFound)
    {
      emit this->ZSignalLog(QString("Frame Sync Header Not Found, Reset Buffer."));
      this->m_ImgLen=0;
      emit this->ZSignalRxBytes(this->m_ImgLen);
      return;
    }
