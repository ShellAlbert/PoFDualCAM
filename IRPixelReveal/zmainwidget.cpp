  ...
  : QWidget(parent)
{
  ...
  this->m_cbTraceCursor->setChecked(g_bTraceCursor);
  QObject::connect(this->m_cbTraceCursor,SIGNAL(toggled(bool)),this,SLOT(ZSlotTraceCursorToggled(bool)));
...
void ZMainWidget::ZSlotTraceCursorToggled(bool bState)
{
    g_bTraceCursor=bState?1:0;
    if(bState)
    {
        emit this->ZSignalLog("Enable Trace Cursor will incline CPU workload.");
    }else{
        emit this->ZSignalLog("Trace Cursor disabled.");
    }
}
...
void ZMainWidget::ZSlotOpenUART()
{
  ...
  if(this->m_btnOpenUART->text()=="Open Port")
    {
      if(this->m_uartRecv->ZOpenUART("/dev/ttyUSB0"))
        {
