...
void ZImageCanvas::ZRedrwFile(const QString &fileName)
{
    ...
    //sync head bytes: 19 87 55 AA 55 AA ............(2+20=22 bytes)
    //search and locate sync head and bypass sync head bytes.
    //robust improvement.
    QByteArray baSyncHead;
    baSyncHead.resize(22);
    datFile.read(baSyncHead.data(),22);
    qint32 startOffset=-1;
    for(qint32 i=0;i<22-1;i++)
    {
        if(baSyncHead.at(i)==(char)0x19 && baSyncHead.at(i+1)==(char)0x87)
        {
            startOffset=i;
            break;
        }
    ...
    if(startOffset<0)
    {
        emit this->ZSignalLog("Failed to find sync head bytes.");
        return;
    }
    emit this->ZSignalLog(QString("SyncHeadOffset=%1").arg(startOffset));

    datFile.seek(startOffset+22); //bypass previous 22 bytes.
    //datFile.seek(23); //bypass previous 23 bytes.
